...
<div class="tags-index">
	...
	'filterModel' => $searchModel,
		...
		'template' => '{item_ident} {update} {delete}',
		'buttons' => [
			...
			if($data['term_id'] != 1){
				...
				'data-pjax' => isset($this->pjax_id)?$this->pjax_id:'0',
                    ...
                    'item_ident' => function ($url, $data, $key) {
			if($data['taxonomy'] == 'item_group'){
			    return Html::a('<span class="glyphicon glyphicon-th-list"></span>', Url::to(['/items/item-ident-name/save', 'id'=>$data['term_id']]), [
				'data-action' => 'item',
				'title' => Yii::t('yii', 'ตั้งชื่อของข้อมูลยืนยันสิ่งของ'),
				'data-pjax' => isset($this->pjax_id)?$this->pjax_id:'0',
			    ]);
			}
		    },        
...
$('#tags-grid-pjax').on('click', 'tbody tr td a', function() {
    ...
    if(action === 'update' || action === 'view' || action === 'item') {
	modalCoreTerm(url);
