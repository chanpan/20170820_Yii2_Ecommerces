...
class CoreFunc {
	...
	if ($option['field_name'] !== NULL) {
		...
		if ($option['field_name'] == 'widget') {
		    eval("\$html = \$form->field(\$model, \$option['$field_name'])->hint(\$option['input_hint'])->{$option['field_name']}({$option['field_class']}, \$options);");
		} else {
		    if (empty($option['input_data'])) {
			eval("\$html = \$form->field(\$model, \$option['$field_name'], \$specific)->hint(\$option['input_hint'])->{$option['field_name']}(\$options);");
		    } else {
			eval("\$html = \$form->field(\$model, \$option['$field_name'], \$specific)->hint(\$option['input_hint'])->{$option['field_name']}({$option['input_data']}, \$options);");
		    }
		}
