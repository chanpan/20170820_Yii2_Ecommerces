...
class SDDrawingv2 extends InputWidget
{
	...
	if(!isset($this->options['saveUrl'])){
	    $this->options['saveUrl'] = Url::to(['//cases/drawing/save-image']);
	} 
	...
	if(!isset($this->options['bgUrl'])){
		...
		$inputID = isset($this->id)?$this->id:$this->name;
	    }
	    $this->options['bgUrl'] = Url::to(['//cases/drawing/bg-image', 'name'=>$inputID]);
	}
	...
	if(isset($inputValue) && $inputValue!=''){
		...
		if(count($fileArr)>1){
			...
			if(stristr($fileName, '.png') == TRUE){
				...
				$this->options['outlineName'] = $fileName;
				$this->options['outlinePath'] = Yii::getAlias('@web').'/drawing/data';
			}
		}
	}
	//echo $this->ui_temp;
        ...
        if ($this->hasModel()) {
            ...
            echo Html::hiddenInput($this->name, $this->value, ['id'=>$inputID]);
        }
	...
	if (isset($this->default_bg) && $this->default_bg!='') {
	    ...
	    if(stristr($fileBg, '.png') == TRUE){
			$view = $this->getView();
			list($width, $height, $type, $attr) = getimagesize(Yii::getAlias('@backend/web/drawing/bg/').$fileBg);
			...
			$view->registerJs("
				...
				$('#{$this->options['drawingName']}').css('background-image', 'url(".Url::to(Yii::getAlias('@web').'/drawing/bg/'.$fileBg).")');
				$('#{$this->options['drawingName']}').css('background-size', bgsize);
				$('#{$this->options['drawingName']}').css('background-position', 'center center');
				$('#{$this->options['drawingName']}').css('background-repeat', 'no-repeat');
			");
	...
	$view->registerJs("
	    ...
	    drawCanvas(".$this->options['width'].", ".$this->options['height'].", '".$this->options['outlineName']."', '".$this->options['outlinePath']."', '".$this->options['outlineBg']."');
