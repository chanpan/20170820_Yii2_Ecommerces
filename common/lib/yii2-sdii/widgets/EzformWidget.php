...
class EzformWidget extends Html {
	...
	if(isset($items['data']) && !empty($items['data'])){
		...
		if ($formatter !== null) {
		    $lines[] = call_user_func($formatter, $index, $label, $name, $checked, $value, $other);
                    ...
                    $other_input = ((isset($other))?Html::textInput($other['attribute'], isset($other['value'])?$other['value']:'', ['class'=>'form-control']):'') . ((isset($other))?" {$other['suffix']}":'');
		    if($inline){
                        $lines[] = static::radio($name, $checked, array_merge($itemOptions, [
                                'labelOptions' => ['class' => 'radio-inline'],
				'value' => $value,
				'label' => '<span>'.($encode ? static::encode($label) : $label).' '.$other_input.'</span> ',
                        ]));
                    } else {
    ...
    public static function checkbox($name, $checked = false, $options = [])
    {
        ...
        if (isset($options['label'])) {
            ...
            $label = "<span>{$options['label']} $other_input</span>";
